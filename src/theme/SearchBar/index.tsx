...
function filterDocusaurusTag(facetFilters: FacetFilters): FacetFilters {
  const lang = useDocusaurusContext().i18n.currentLocale === 'en-US' ? 'en' : 'zh';
  const version = getStorage('VERSION') ? getStorage('VERSION').replaceAll('.', '_') : 'current'

  return facetFilters?.map(facetFiltersItem => {
    if (Array.isArray(facetFiltersItem)) {

      return filterDocusaurusTag(facetFiltersItem)

    }
    if (facetFiltersItem.includes('docusaurus_tag')) {
      const tags = facetFiltersItem.split(':')[1]?.split('-')
      if (tags[1] && tags[2] && (lang !== tags[1] || !['community', version].includes(tags[2]))) {
        return null
      }
      return facetFiltersItem
    }
    return facetFiltersItem
  })?.filter(item => item)
}
...
function useSearchParameters({
  ...
  return {
    ...
    facetFilters: filterDocusaurusTag(facetFilters),
  };
}
